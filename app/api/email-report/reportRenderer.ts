...
function drawAssessmentBox(page: PDFPage, data: SkinCheckerReportData, x: number, y: number, w: number, h: number, fonts: Fonts) {
  ...
  const skinScore = data.skinScore;
  const hasSkinScore = typeof skinScore?.total === "number";
  ...
  drawText(page, assessmentTitle(colour), x + 14, y + h - 43, {
    font: fonts.bold,
    size: hasSkinScore ? 14.5 : 17,
    color: c.text,
  });

  if (hasSkinScore) {
    const dividerY = y + h - 57;

    page.drawLine({
      start: { x: x + 14, y: dividerY },
      end: { x: x + w - 14, y: dividerY },
      thickness: 0.7,
      color: c.border,
    });
    ...
    drawText(page, `${skinScore.total}/100`, x + 14, y + h - 93, {
      font: fonts.bold,
      size: 18,
      color: c.text,
    });

    const gradeText = `${clean(skinScore.grade, "Risk")} Risk`.toUpperCase();
    const gradeW = textWidth(fonts.bold, gradeText, 8.6);

    drawText(page, gradeText, x + w - 14 - gradeW, y + h - 88, {
      font: fonts.bold,
      size: 8.6,
      color: c.text,
    });
